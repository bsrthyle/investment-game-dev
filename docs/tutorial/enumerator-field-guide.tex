...
\setlength{\fboxsep}{0pt}\setlength{\fboxrule}{0.6pt}%
\fcolorbox{black!20}{white}{\includegraphics[width=0.7\linewidth]{img/admin-sync.png}}\\[2cm]
...
\keybox{Credentials}{%
\begin{tabular}{@{}ll@{}}
\textbf{Admin PIN} & \texttt{CIMMYT1234} \\
\textbf{Sync server address} & \texttt{https://fertilizer-game-server.bismignot.workers.dev} \\
\textbf{Sync (bearer) token} & \rule{5cm}{0.4pt}\quad(from your study coordinator) \\
\end{tabular}\\[4pt]
Keep the bearer token private. On most deployments the coordinator pre-enters
it on each tablet, so you will not need to type it.}
...
\smallshot{img/admin-pin.png}{Enter the Admin PIN (\texttt{CIMMYT1234}).}
...
\setlength{\fboxsep}{0pt}\setlength{\fboxrule}{0.6pt}%
\fcolorbox{black!25}{white}{\includegraphics[width=\linewidth]{img/admin-diagnostics.png}}\\[2pt]
{\small\itshape\color{inkgrey} \textbf{Diagnostics} --- check the app is offline-ready,
storage is healthy, and battery is fine.}
\end{minipage}\hfill
\begin{minipage}{0.49\linewidth}\centering
\setlength{\fboxsep}{0pt}\setlength{\fboxrule}{0.6pt}%
\fcolorbox{black!25}{white}{\includegraphics[width=\linewidth]{img/admin-export.png}}\\[2pt]
...
\end{itemize}
The token-to-Naira rate is fixed at \textbf{5 NGN per token} (not editable). Then tap \textbf{Start session}.
...
The participant now: chooses the \textbf{language}, watches a short \textbf{tutorial video} in that
language, sees a short \textbf{picture lesson} about chance,
...
\screenshot{img/final-payout.png}{The final earnings screen. Tokens convert at \textbf{5 NGN per token};
here \textbf{253 tokens = 1{,}265 NGN}. Pay the Naira figure shown. Tap \textbf{Continue} and the
``thank you'' screen ends the session.}
...
version; the game is also played in Hausa. Keep this guide with the tablet.\\[4pt]
This game is adapted from the MIT-licensed \emph{Investment Game} originally developed by the Alliance of
Bioversity International \& CIAT and IFPRI; the CIMMYT version reframes it for fertilizer investment under
rainfall and price uncertainty.}
\end{center}
